...
| 20 Aug 2026 | **The weave ground — WITHDRAWN same day.** Renato: it only worked on the landing page, everywhere else it read badly. The ground weave is removed from `body`, `.evx-light`, `.evx-dark`, `.evx-band` and `.evx-pit`; grounds are flat colour again. **The hero field keeps its weave** — `.evx-field__weave` is the one Renato liked and is untouched. The lesson worth keeping: texture at page scale competes with the product instead of framing it, and the depth this site needs comes from rhythm, tone and photography, not from a surface treatment. Superseded entry below, kept for the record. | WITHDRAWN |
